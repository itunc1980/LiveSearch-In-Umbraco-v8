(function () {
    ...
    function publishResultController(
        $scope,
        overlayService,
        editorService) {
        ...
        function showFailure(item) {

            if (vm.isModal || item.Success === true || _.isEmpty(item.Message)) {
                return;
            }

            overlayService.open({
                title: "Item failure",
                view: Umbraco.Sys.ServerVariables.application.applicationPath + "App_Plugins/uSyncPublisher/components/failureOverlay.html",
                item: item,
                itemType: getTypeName(item.ItemType),
